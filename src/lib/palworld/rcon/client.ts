...
export default class RCONClient {
  ...
  connect(password: string): Promise<RCONClient> {
    ...
    return new Promise<RCONClient>((resolve, reject) => {
      ...
      const onData = (data: Buffer) => {
        ...
        if (
          packet.type === RCONPacketType.COMMAND &&
          packet.requestId === authPacket.requestId
        ) {
  ...
  sendCommand(command: string): Promise<string> {
    ...
    const endPacket = RCONPacket.createFrom(
      cmdPacket.requestId + 1,
      RCONPacketType.COMMAND_END,
      ""
    );
    ...
    const onDataFunc = (
      ...
      return (data: Buffer) => {
        ...
        if (
          packet.type === RCONPacketType.RESPONSE &&
          packet.requestId === cmdPacket.requestId
        ) {
        ...
        if (
          packet.type === RCONPacketType.RESPONSE &&
          packet.requestId === endPacket.requestId
        ) {
